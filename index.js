...
const METROPOLE_API_HOST = 'api.metropole.gg';
...
function cleanupUserDataLocks() {
  if (!fs.existsSync(USER_DATA_DIR)) return;
  for (const entry of fs.readdirSync(USER_DATA_DIR)) {
    if (entry === 'lockfile' || entry.startsWith('Singleton')) {
      try { fs.rmSync(path.join(USER_DATA_DIR, entry), { force: true, recursive: true }); } catch {}
    }
  }
}
...
async function isDiscordLoggedIn(context) {
  ...
  try {
    ...
    return await page
      .locator('[data-list-id="guildsnav"], nav[aria-label*="Servidor" i], nav[aria-label*="Server" i]')
      .first()
...
async function ensureLoggedIn() {
  const hasSession = fs.existsSync(path.join(USER_DATA_DIR, 'Default'));
  let context = await launchContext(hasSession);
  let loggedIn = await isDiscordLoggedIn(context);

  if (!loggedIn) {
    log('Discord NÃO logado. Reabrindo browser com interface para login manual.');
    await context.close();
    await sleep(3000);
    context = await launchContext(false);
    const loginPage = await context.newPage();
    await loginPage.goto('https://discord.com/login', { waitUntil: 'domcontentloaded' });
    await waitForEnter('>> Faça login no Discord e pressione ENTER aqui quando terminar... ');
    await loginPage.close();
    loggedIn = await isDiscordLoggedIn(context);
    if (!loggedIn) { await context.close(); throw new Error('Login no Discord não detectado.'); }
    log('Login detectado. Sessão salva em user-data/.');
  }
  return context;
}

async function clickChannelButton(context, buttonText) {
  const page = await context.newPage();
  log(`Navegando para o canal para clicar em "${buttonText}"`);
  await page.goto(DISCORD_CHANNEL_URL, { waitUntil: 'domcontentloaded', timeout: 45000 });

  try {
    await page.waitForSelector('[data-list-id="chat-messages"], main[class*="chatContent"]', { timeout: 20000 });
  } catch {
    log('Timeout esperando container de mensagens (segue mesmo assim).');
  }
  await page.waitForTimeout(4000);
  await page.keyboard.press('End').catch(() => {});
  await page.waitForTimeout(2000);

  const rx = new RegExp(buttonText.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i');
  const strategies = [
    () => page.getByRole('button', { name: rx }),
    () => page.locator(`button:has-text("${buttonText}")`),
    () => page.locator(`[role="button"]:has-text("${buttonText}")`),
  ...
  if (!clicked) {
    ...
    const shotPath = path.join(__dirname, `debug-${stamp}.png`);
    const htmlPath = path.join(__dirname, `debug-${stamp}.html`);
    await page.screenshot({ path: shotPath, fullPage: true }).catch(() => {});
    fs.writeFileSync(htmlPath, await page.content().catch(() => ''));
    log(`Debug salvo: ${shotPath} e ${htmlPath}`);
    throw new Error(`Botão "${buttonText}" não localizado no canal.`);
  }

  log(`Clique em "${buttonText}" realizado.`);
  await page.waitForTimeout(3000);
  await page.close();
}

async function clickButtonFlow(buttonText) {
  const context = await ensureLoggedIn();
  try { await clickChannelButton(context, buttonText); }
...
class DutyDetector extends EventEmitter {
  // events: 'status' ({status, text, source}), 'no-connection', 'reconnected', 'stopped'
  constructor() {
    super();
    ...
    try {
      ...
      session.on('Runtime.executionContextCreated', (ev) => {
        ...
        if (aux.isDefault && aux.frameId) {
          ...
          const short = (ctx.origin || ctx.name || '').slice(0, 60);
          log(`[mainCtx] frame=${aux.frameId.slice(0,6)} ctxId=${ctx.id} origin=${short}`);
        }
      });
...
// clickButton(texto) é injetável para permitir teste sem Playwright/Discord.
function wireDetector(detector, clickButton) {
  ...
  detector.on('status', ({ status, text, source }) => {
    ...
    else if (status === 'off-duty') doClose('off-duty');
  });
  ...
  return {
    get pontoOpen() { return pontoOpen; },
    doOpen,
    doClose,
  };
}
...
async function main() {
  // Sem prazo: o detector sonda a rota indefinidamente e anexa assim que o FiveM abrir.
  log(`Aguardando FiveM em ${NUI_URL}... (Ctrl+C para sair)`);

  const detector = new DutyDetector();
  wireDetector(detector, clickButtonFlow);

  process.on('SIGINT', () => { detector.stop(); setTimeout(() => process.exit(0), 500); });

  await detector.start();
}

module.exports = { DutyDetector, wireDetector };
